@foreach ($invoiceLines as $key => $invoiceLine)
    <cac:InvoiceLine>
        ...
        <cbc:InvoicedQuantity unitCode="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->unit_measure->code) }}">
            {{ preg_replace("/[\r\n|\n|\r]+/", '', number_format($invoiceLine->invoiced_quantity, 6, '.', '')) }}
        ...
        <cbc:LineExtensionAmount currencyID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $company->type_currency->code) }}">
            {{ preg_replace("/[\r\n|\n|\r]+/", '', number_format($invoiceLine->line_extension_amount, 2, '.', '')) }}
        ...
        @if (isset($invoiceLine->type_generation_transmition))
            <cac:InvoicePeriod>
                ...
                <cbc:DescriptionCode>
                    {{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->type_generation_transmition->id) }}
                </cbc:DescriptionCode>
                <cbc:Description>
                    {{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->type_generation_transmition->name) }}
                </cbc:Description>
            </cac:InvoicePeriod>
        @endif
        @if ($typeDocument->id != '11')
            <cbc:FreeOfChargeIndicator>
                {{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->free_of_charge_indicator) }}
            </cbc:FreeOfChargeIndicator>
        @endif
        @if (preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->free_of_charge_indicator) === 'true')
            <cac:PricingReference>
                ...
                @else   --}}
                    <cbc:PriceAmount
                        currencyID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $company->type_currency->code) }}">
                        {{ preg_replace("/[\r\n|\n|\r]+/", '', number_format(1, 2, '.', '')) }}</cbc:PriceAmount>
                    {{--                @endif  --}}
                </cac:AlternativeConditionPrice>
            </cac:PricingReference>
        @endif
        ...
        <cac:Item>
            ...
            <cac:StandardItemIdentification>
                <cbc:ID
                    schemeID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->type_item_identification->code) }}"
                    schemeName="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->type_item_identification->name) }}"
                    schemeAgencyID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->type_item_identification->code_agency) }}">
                        ...
                        <cac:AgentParty>
                            <cac:PartyIdentification>
                                <cbc:ID schemeAgencyID="195"
                                    schemeID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->agentparty_dv) }}"
                                    schemeName="31">{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->agentparty) }}
                                </cbc:ID>
                            </cac:PartyIdentification>
                        </cac:AgentParty>
                    </cac:PowerOfAttorney>
            ...
            @if ($request->isTransport)
                @if ($invoiceLine->is_RNDC)
                    <cac:AdditionalItemProperty>
                        ...
                        <cbc:Value>{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->RNDC_consignment_number) }}
                        </cbc:Value>
                    </cac:AdditionalItemProperty>
                    <cac:AdditionalItemProperty>
                        <cbc:Name>02</cbc:Name>
                        <cbc:Value>{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->internal_consignment_number) }}
                        </cbc:Value>
                    </cac:AdditionalItemProperty>
                    <cac:AdditionalItemProperty>
                        <cbc:Name>03</cbc:Name>
                        <cbc:Value>{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->value_consignment) }}
                        </cbc:Value>
                        <cbc:ValueQuantity
                            unitCode="{{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->unit_measure_consignment->code) }}">
                            {{ preg_replace("/[\r\n|\n|\r]+/", '', $invoiceLine->quantity_consignment) }}
                        </cbc:ValueQuantity>
                    </cac:AdditionalItemProperty>
                @endif
            @endif
        ...
        <cac:Price>
            ...
            @else   --}}
            <cbc:PriceAmount currencyID="{{ preg_replace("/[\r\n|\n|\r]+/", '', $company->type_currency->code) }}">
                {{ preg_replace("/[\r\n|\n|\r]+/", '', number_format($invoiceLine->free_of_charge_indicator === 'true' ? 0 : $invoiceLine->price_amount, 2, '.', '')) }}
            </cbc:PriceAmount>
            {{--            @endif  --}}
