        ...
        <td style="width: 23%;" class="text-center vertical-align-top">
            <div id="reference">
                @if ($request->type_document_id == 3)
                    <p style="font-weight: 700;"><strong>FACTURA ELECTRONICA DE CONTINGENCIA No</strong></p>
                @else
            ...
            <div id="empresa-header1">
                @if (isset($request->ivaresponsable))
                    @if ($request->ivaresponsable != $company->type_regime->name)
                        NIT: {{ $company->identification_number }}-{{ $company->dv }} -
                        {{ $company->type_regime->name }} - {{ $request->ivaresponsable }}
                        {{-- - Obligación: {{$company->type_liability->name}} --}}
                    @else
                        NIT: {{ $company->identification_number }}-{{ $company->dv }} -
                        {{ $company->type_regime->name }}
                        {{-- - Obligación: {{$company->type_liability->name}} --}}
                    @endif
                @else
                    NIT: {{ $company->identification_number }}-{{ $company->dv }} -
                ...
                @endif
                Resolución de Facturación Electrónica No. {{ $resolution->resolution }} de
                {{ $resolution->resolution_date }}<br>
                Prefijo: {{ $resolution->prefix }}, Rango
                {{ $resolution->from }} Al {{ $resolution->to }} - Vigencia Desde: {{ $resolution->date_from }}
                Hasta: {{ $resolution->date_to }}<br>
                ...
                @if (isset($request->establishment_municipality))
                    {{ $municipality->findOrFail($request->establishment_municipality)['name'] }} -
                    {{ $municipality->findOrFail($request->establishment_municipality)['department']['name'] }} -
                @else
                    {{ $company->municipality->name }} -
                    {{ $municipality->findOrFail($company->municipality->id)['department']['name'] }} -
                @endif
                ...
                {{-- @if (isset($request->establishment_email))
                    ...
                    @endif --}}
                <br>
                Teléfono - {{ $company->phone }} E-mail: {{ $company->email_pdf }} <br>
                @if (isset($request->seze))
