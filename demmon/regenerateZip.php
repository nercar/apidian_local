	...
	function getData($conSQLLoc, $tienda, $cnx, $number, $prefix, $newtype = 0)
	{
		...
		if ($heads === false) {
			...
			while ($row = sqlsrv_fetch_array($heads, SQLSRV_FETCH_ASSOC)) {
				...
				$jsObj->prefix = 'F132';
				$jsObj->number = 35175;
				$sqlTxt = "SELECT * FROM documentS WHERE prefix = 'F132' AND NUMBER = 34867 AND state_document_id = 1 ORDER BY id DESC LIMIT 1";
				$document = $cnx->query($sqlTxt)->fetchAll(PDO::FETCH_ASSOC);
