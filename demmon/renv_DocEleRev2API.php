        ...
        try {
            if ($argc > 1) {
                if ($argc == 5) {
                    ...
                    $prefix = '';
                    if ($argc == 4)
                        $fecha = 'CAST(created_at AS DATE) >= CAST(DATEADD(day, -' . $argv[3] . ', GETDATE()) AS DATE) AND CAST(created_at AS DATE) < CAST(GETDATE() AS DATE)';
                    else
                        $fecha = 'CAST(created_at AS DATE) >= CAST(DATEADD(day, -3, GETDATE()) AS DATE) AND CAST(created_at AS DATE) < CAST(GETDATE() AS DATE)';
                }
                if ($argc == 6) {
                } else {
                }
                ...
                if ($number != '0') {
                    ...
                    if ($pend === false) {
                        ...
                        while ($row = sqlsrv_fetch_array($pend, SQLSRV_FETCH_ASSOC)) {
                            getData($conSQLLoc, $ipserv, $cnx, $row['folio'], $row['prefijo'], $site);
                        }
                    }
                }
            }
    ...
    if ($err) {
        ...
        if ($valido) {
            ...
            if ($identification_number != "222222222222") {
                $url  = "http://$site/api/ubl2.1/send-email";
